		...
		<el-card class="box-card">
			...
			<template>
				<el-table :data="tableData" v-loading="listLoading" element-loading-text="数据加载中..." border style="width:100%;">
					...
					<el-table-column prop="Content" label="APP推送内容" min-width="300"></el-table-column>
...
import { apppushtpllist,apppushtplisused,apppushtpledit,varcontrast } from 'api/api.js';
export default {
	data(){
		return {
			page:1,
	...
	methods: {
		...
		freezeDtl(id,isUsed){  //禁用、启用
			...
			apppushtplisused(params).then((res) => {
				if( res.Ret == 200 ){
					this.$message.success( res.Msg );
					this.getapppushtpllist();
				}
				if(res.Ret == 100){
					this._sendCode(params,apppushtplisused).then(res=>{
							if( res.Ret === 200 ){
								this.$message.success( res.Msg );
								this.getapppushtpllist();
							}
      	...
      	editSubmit(){  //修改（提交）
      		...
      		apppushtpledit(params).then((res) => {
      			if(res.Ret == 200){
				  ...
				  if(res.Ret == 100){
					this._sendCode(params,apppushtpledit).then(res=>{
							if( res.Ret === 200 ){
								this.showDialog_edit = false;
								this.$message.success( res.Msg );
								this.getapppushtpllist();
