		...
		<el-card class="box-card">
			...
			<template>
				<el-table :data="tableData" v-loading="listLoading" element-loading-text="数据加载中..." border style="width:100%;">
					...
					<el-table-column prop="Content" label="短信内容" min-width="300"></el-table-column>
					<!--<el-table-column label="审核结果">
						<template slot-scope="scope">
							<span>{{scope.row.Checked==0?"未通过审核":"通过审核"}}</span>
		  	    		</template>
					</el-table-column>-->
					<el-table-column label="状态">
						<template slot-scope="scope">
	  	    		        <el-switch v-if="scope.row.Checked==1" v-model="scope.row.State" active-text="启用" active-value="1" inactive-text="禁用" inactive-value="0" @change="freezeDtl(scope.row.Id,scope.row.State)"></el-switch>
	  	    		        <el-switch v-else disabled v-model="scope.row.State" active-text="启用" active-value="1" inactive-text="禁用" inactive-value="0"></el-switch>
		  	    		</template>
					</el-table-column>
...
import { smstemplatelist,smstemplateisused,smstemplateedit,varcontrast } from 'api/api.js';
export default {
	data(){
		return {
			page:1,
	...
	methods: {
		...
		freezeDtl(id,isUsed){  //禁用、启用
			...
			smstemplateisused(params).then((res) => {
				if( res.Ret == 200 ){
					this.$message.success( res.Msg );
					this.getsmstemplatelist();
				}
				if(res.Ret == 100){
					this._sendCode(params,smstemplateisused).then(res=>{
							if( res.Ret === 200 ){
								this.$message.success( res.Msg );
								this.getsmstemplatelist();
							}
      	...
      	editSubmit(){  //修改（提交）
      		...
      		smstemplateedit(params).then((res) => {
      			if(res.Ret == 200){
				...
				if(res.Ret == 100){
					this._sendCode(params,smstemplateedit).then(res=>{
							if( res.Ret === 200 ){
								this.showDialog_edit = false;
								this.$message.success( res.Msg );
								this.getsmstemplatelist();
