				...
				<div slot="header" class="header">
					<el-form :inline="true" :model="form" @submit.native.prevent>
						...
						<el-form-item style="float:right;">
							<el-button type="primary" size="small" @click="isTransform=false">查看图像</el-button>
						</el-form-item>
					</el-form>
				</div>
				<template>
					<el-table :data="tableData" v-loading="listLoading" element-loading-text="数据加载中..." border style="width:100%;">
						...
						<el-table-column prop="TotalNumRegister" label="总注册人数"></el-table-column>
						<el-table-column prop="TotalNumLoan" label="总借款用户"></el-table-column>
						<el-table-column prop="TotalMoneyLoan" label="总借款金额"></el-table-column>
						<el-table-column prop="TotalMoneyNomalReport" label="征信报告收入"></el-table-column>
						<el-table-column prop="TotalMoneyRefuseReport" label="查看报告收入"></el-table-column>
						<el-table-column prop="TotalMoneyOverdue" label="逾期总金额"></el-table-column>
					</el-table>
				</template>
...
<script>
	...
	import { dailytotaldata } from 'api/api.js';
	...
	export default {
		data() {
				return {
					...
					tableData: [],
					form: {
						dateValue: [http.dateFormatter(new Date(new Date().getTime() - 10 * 24 * 60 * 60 * 1000), false), http.dateFormatter(new Date(), false)]
					},
					isDisplay: false,
					dateData: [],
					regData: [],
			...
			methods: {
				...
				getdailytotaldata() { //获取列表
					...
					dailytotaldata(params).then((res) => {
				...
				lookecharts() {
					...
					this.tableData.forEach(item => {
						...
						this.regData.push(item.TotalNumRegister);
						this.authData.push(item.TotalNumLoan);
					});
