			...
			<el-card class="box-card">
				...
				<template>
					<el-table :data="tableData" v-loading="listLoading" element-loading-text="数据加载中..." border style="width:100%;">
						...
						<el-table-column prop="MoneyOverdueAdd" label="新增逾期金额"></el-table-column>
						<el-table-column prop="MoneyBreachOverdueAdd" label="新增违约金"></el-table-column>
						<el-table-column prop="MoneyOverduemoneyAdd" label="新增滞纳金"></el-table-column>
						<el-table-column prop="MoneyDrcollectReturn" label="当日催回金额"></el-table-column>
						<el-table-column prop="MoneyDiffOverdue" label="逾期差(催回金额-新增逾期金额)"></el-table-column>
					</el-table>
				</template>
...
import { dailyoverduedata } from 'api/api.js';
...
export default {
	...
	methods: {
		...
		getdailyoverduedata(){  //获取列表
			...
			dailyoverduedata(params).then((res) => {
		...
		handleSizeChange(val){    //分页页码改变
	   	...
	   	lookecharts(){
	   		this.dateData=[];
			this.MoneyOverdueAddData=[];
			this.MoneyDrcollectReturnData=[];
	   		this.tableData.forEach(item=>{
				this.dateData.push( item.CreateTime );
				this.MoneyOverdueAddData.push( item.MoneyOverdueAdd );
				this.MoneyDrcollectReturnData.push( item.MoneyDrcollectReturn );
			});
			this.dateData=this.dateData.reverse();
			this.MoneyOverdueAddData=this.MoneyOverdueAddData.reverse();
			this.MoneyDrcollectReturnData=this.MoneyDrcollectReturnData.reverse();
			this.drawLineChart();
	   	},
	   	drawLineChart() {
		    ...
		    this.chartLine.setOption({
			    ...
			    series:[
			        ...
			        },
			        {
			            ...
			            areaStyle: {normal: {}},
			            data:this.MoneyDrcollectReturnData
			        }
			    ]
			});
		}
