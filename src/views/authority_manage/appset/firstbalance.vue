<template>
	<div>
		<el-card class="box-card">
			<div slot="header" class="clearfix">首借额度设置</div>
			<el-form :model="addForm" :rules="rules" label-width="120px" ref="addForm">
				<el-form-item label="首借额度" prop="Amount">
		          	...
		          	<span style="padding:11px 20px; color:#1989FA; border:1px solid #108EE9; border-radius:5px;">非白名单用户通过授信默认获得的借款额度。 如无授信环节，则首借额度为通过风控系统审核给用户的建议借款额度。</span>
		        </el-form-item>
...
<script>
	import { getfirstbalance,setfirstbalance } from 'api/api.js';
	export default {
		data() {
			return {
				addForm:{
	     	...
	     	addSubmit(){
	     		this.$refs.addForm.validate((valid) => {
					if( valid ){
						...
						setfirstbalance(this.addForm).then((res) => {
							if( res.Ret === 200 ){
