					...
					<el-table :data="tableDataA" v-loading="listLoading" :show-header="false" element-loading-text="数据加载中..." border>
						<el-table-column  label=""  min-width="350">
							 <template slot-scope="scope">
                                ...
                                <span style="margin-left: 10px">{{ scope.row[0].value}}</span>
							</template>
						</el-table-column>
						<el-table-column prop="value" label=""></el-table-column>
						<el-table-column label="" min-width="350">
							 <template slot-scope="scope">
								<span style="margin-left: 10px">{{ scope.row[1] ? scope.row[1].title +':' : ''}}</span>
                                <span style="margin-left: 10px">{{scope.row[1] ?  scope.row[1].value : ''}}</span>
							</template>
						</el-table-column>
						<el-table-column prop="value" label=""></el-table-column>
					</el-table>
		...
		methods: {
			...
			getList() {
				...
				getFKFKCreditReport(params).then((res)=>{
					...
					let itemArr = [];
					const TCRData = testdata1.TCReport.Report.data.result;
					//收集返回的数据型号
					...
					// 	return;
					// }
                    const UDRData = testdata1.UDReport.Report.boby;
                    console.log("UDRData",UDRData);
					const info = testdata1.UDReport.Report.boby.request_info;
					console.log(UDRData);
                    ...
                    // this.finger = UDRData.devices_list ? UDRData.devices_list : [];
                    this.finger = UDRData.devices_list.map(ele =>{
                        return ele.device_detail;
                    })
