		...
		<el-card class="box-card">
			...
			<template>
				<el-table :data="tableData" v-loading="listLoading" element-loading-text="数据加载中..." border>
					...
					<el-table-column prop="CallTime" label="呼叫时间" min-width="120"></el-table-column>
					<el-table-column label="响铃时长">
                        <template slot-scope="scope">{{scope.row.RingTime}}秒</template>
                    </el-table-column>
					<el-table-column  label="通话时长">
                        <template slot-scope="scope">{{(scope.row.Duration/60).toFixed(1)}}分钟</template>
                    </el-table-column>
					<el-table-column prop="Caller" label="主叫"></el-table-column>
					<el-table-column prop="Callee" label="被叫"></el-table-column>
					<el-table-column prop="CallUuid" label="录音" width="220px" :render-header="renderHeader2">
					...
					<el-table-column prop="State" label="催回状态">
                        <template slot-scope="scope">
                            {{scope.row.State =='未还'?'未结清':'已结清'}}
                        </template>
                    </el-table-column>
...
export default {
	...
	methods: {
        ...
        viewdetails(row){  //查看详情
           console.log(row);
           if(row.State == '未还'){
                let {href} = this.$router.resolve({path:`/collectiondtl/${row.Uid}/${row.LoanId}`});
