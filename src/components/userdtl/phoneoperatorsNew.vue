	...
	<div v-if="isshow" style="margin:0 auto;">
		<div id="header">
			...
			<p style="font:20px '楷体'; padding:0 40px; box-sizing:border-box;">
				<!--<span style="float:left;">报告编号：{{info._Head.SearchId}}</span>-->
				<span style="float:right;">报告生成时间：{{info._Head.ReportTime}}</span>
			</p>
		</div>
		<h1 class="th1">输入信息</h1>
		<div style="font:15px/40px '微软雅黑'; padding:10px 20px; box-sizing:border-box; background:#f9f9f9; border:1px solid #eaeaea; overflow:hidden;">
			<el-col :span="8">
				<label class="tlabel">用户姓名：</label>
				<span class="tspan">{{info.Input.Name}}</span>
			</el-col>
			<el-col :span="8">
				<label class="tlabel">身份证号：</label>
				<span class="tspan">{{info.Input.IdCard}}</span>
			</el-col>
			<el-col :span="8">
				<label class="tlabel">手机号码：</label>
				<span class="tspan">{{info.Input.Phone}}</span>
			</el-col>
			<div v-for="(item,index) in info.Input.Emergency" :key="index">
				<el-col :span="8">
					<label class="tlabel">紧急联系人{{item.No}}：</label>
					<span class="tspan">{{item.Name}}</span>
				</el-col>
				<el-col :span="8">
					<label class="tlabel">关系：</label>
					<span class="tspan">{{item.Relation=='father'?'父亲':item.Relation=='mother'?'母亲':item.Relation=='spouse'?'配偶':item.Relation=='brother'?'兄弟':item.Relation=='sister'?'姐妹':item.Relation=='children'?'子女':item.Relation=='friend'?'朋友':item.Relation=='colleague'?'同事':'其他'}}</span>
				</el-col>
				<el-col :span="8">
					<label class="tlabel">联系电话：</label>
					<span class="tspan">{{item.Phone}}</span>
				</el-col>
			</div>
		</div>
...
<script>
	import {operatorrpt,operatorrptcalllog} from 'api/api.js';
	export default {
		props:{
      		uid:{
        		type:String,
