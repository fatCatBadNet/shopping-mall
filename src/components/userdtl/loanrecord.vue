		...
		<template>
			<el-table :data="tableData" v-loading="listLoading" element-loading-text="数据加载中..." border>
				...
				<el-table-column label="借款期限" min-width="150">
					<template slot-scope="scope">
  	    		        <label style="display:inline-block; width:40px;">{{scope.row.LoanTermCount}}天</label>
  	    		        <span style="color:#999;" v-if="scope.row.ReplyTime&&scope.row.EndDate">{{scope.row.ReplyTime}}至{{scope.row.EndDate}}</span>
	  	    		</template>
				</el-table-column>
				...
				<el-table-column label="操作" width="300">
					<template slot-scope="scope">
  	    		        ...
  	    		        <!-- <a :href="scope.row.LoanAgreementUrl" target="_blank" class="editwordSty">借款协议</a>
  	    		        <a :href="scope.row.ServiceAgreementUrl" target="_blank" class="editwordSty">服务协议</a> -->
	  	    		</template>
				</el-table-column>
			</el-table>
	  	...
	  	<el-dialog title="还款计划" :visible.sync="showDialog" :close-on-click-modal="true" :center="true" v-dialogDrag custom-class="dialogclass" width="1100px">
			<el-table :data="scheduleData" v-loading="scheduleLoading" element-loading-text="数据加载中..." border style="width:100%; text-align:left;">
				...
				<el-table-column prop="CapitalAmount" label="本金"></el-table-column>
				<el-table-column prop="TaxAmount" label="利息"></el-table-column>
				<el-table-column prop="OverdueBreachOfAmount" label="违约金"></el-table-column>
				<el-table-column prop="OverdueMoneyAmount" label="滞纳金"></el-table-column>
				<el-table-column prop="UnReturnMoney" label="应还金额"></el-table-column>
				<el-table-column prop="ReturnMoney" label="已还金额"></el-table-column>
